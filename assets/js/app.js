...
Device.prototype.setUA = function (str) {
    this.ua = str;
};

Device.prototype.isSp = function () {
...
Device.prototype.isPc = function () {
    return this.getDevice() === "other";
};

Device.prototype.getDevice = function () {
    if (this.isIosMobile() || this.isAndroidMobile()) {
        return 'sp';
...
Device.prototype.isIpod = function () {
...
Device.prototype.isIosMobile = function () {
    return this.isIphone() || this.isIpod();
};

Device.prototype.isIosTablet = function () {
    return this.isIpad();
};
...
Device.prototype.isAndroidMobile = function () {
    return this.isAndroid() && this.ua.indexOf('Mobile') > 0;
};

Device.prototype.isAndroidTablet = function () {
    return this.isAndroid() && this.ua.indexOf('Mobile') === -1;
};
...
var spNav = require('./spNav/spNav');
var scrollSpy = require('./scrollSpy/scrollSpy');

var isPcWidth = function isPcWidth() {
    return parseInt($(window).width(), 10) >= 1000;
};
...
$(function () {
    smoove({
        offset: function offset() {
            return isPcWidth() ? 87 : 49;
        }
    });
    spNav();
    scrollSpy({
        offset: function offset() {
            return isPcWidth() ? 87 : 50;
        }
    });
});

},{"./device/device":4,"./scrollSpy/scrollSpy":6,"./spNav/spNav":7,"bangs-js":1,"jquery":2,"smoove":3}],6:[function(require,module,exports){
(function (global){
"use strict";
...
(function (definition) {
    ...
    function Module(opt, moduleRoot) {
        ...
        // options
        this.opt = {
            nav: !isUndefined(opt.nav) ? opt.nav : ".js-scrollSpy__nav",
            mark: !isUndefined(opt.mark) ? opt.mark : ".js-scrollSpyMark",
            offset: !isUndefined(opt.offset) ? opt.offset : 80
        };

        // elements
        this.$root = $(moduleRoot);
        this.$nav = this.$root.find(this.opt.nav);
        this.$mark = this.$root.find(this.opt.mark);

        // state
        this.windowHeight = root.innerHeight;
        this.itemLength = this.$nav.length;
        this.pos = [];
        this.currentIndex = 0;
        this.resizeTimer = null;
    ...
    Module.prototype.onresize = function () {
        ...
        $(root, document).on('resize', function (e) {
            _this.onresizeHandler(e);
        });
    };

    Module.prototype.onscroll = function () {
        var _this2 = this;

        $(root).on('scroll', function (e) {
            _this2.onscrollHandler(e);
        });
    };

    Module.prototype.ondocumentresize = function () {
        var _this3 = this;

        var documentHeight = this.getDocumentHeight();

        setInterval(function () {
            var currentDocumentHeight = _this3.getDocumentHeight();

            if (documentHeight === currentDocumentHeight) return;
    ...
    Module.prototype.onresizeHandler = function (evnet) {
        var _this4 = this;
    ...
    Module.prototype.onscrollHandler = function (evnet) {

        var scrollY = $(root).scrollTop();

        for (var i = 0; i < this.pos.length; i++) {

            if (this.pos[i].inView <= scrollY && scrollY <= this.pos[i].outView) {
                if (i !== this.currentIndex) {
                    this.currentIndex = i;
                    this.updateClass(this.currentIndex);
                }
            }
        }
        return false;
    ...
    Module.prototype.setPos = function (evnet) {
        var _this5 = this;

        this.pos = [];

        this.$nav.each(function (index, val) {

            // スクロール0から1個目までの領域を作るために
            // 0個目を常に作る
            if (index === 0) {
                var _target = _this5.$nav.first().attr("href");
                var firstInView = $(_target).offset().top;
                _this5.pos.push({ inView: 0, outView: firstInView });
            }

            var target = $(val).attr("href");
            var $target = $(target);

            var offset = _this5.opt.offset;
            if (typeof offset === 'function') offset = offset();
            var inView = $target.offset().top - offset;
            var outView = inView + $target.outerHeight(true) - offset;

            // 最後の要素は最後までスクロールした値とinViewの小さい方にする
            if (index === _this5.$nav.length - 1) {
                var scrollEnd = _this5.getScrollEnd();
                inView = Math.min(inView, scrollEnd);
            }
    ...
    Module.prototype.updateClass = function (index) {
        this.$root.removeClass(this.makeRemoveClassStr()).addClass("is-" + this.currentIndex);
    };

    Module.prototype.makeRemoveClassStr = function () {
        var classStr = "";
    ...
    Module.prototype.getDocumentHeight = function () {
        return Math.max.apply(null, [document.body.clientHeight, document.body.scrollHeight, document.documentElement.scrollHeight, document.documentElement.clientHeight]);
    };

    // ページ最下部までのスクロール量
    Module.prototype.getScrollEnd = function () {
...
var $toggleBtn = $(".js-menuToggle");
var $openBtn = $(".js-menuOpen");
var $closeBtn = $(".js-menuClose");
var $menu = $(".js-spNav");
var $menuLinks = $menu.find('a');
var $menuBtn = $menu.find(".js-smoove");
var openClass = "s-open";
var isOpen = false;
var resizeTimer = null;

module.exports = function () {
    // if( parseInt($(window).width(), 10) >= 1000 ) return;
    init();
};
...
function onresizeHandler(evnet) {
    if (resizeTimer !== false) clearTimeout(resizeTimer);

    resizeTimer = setTimeout(function () {
        if (parseInt($(window).width(), 10) < 980) return;
        close();
    }, 200);
}
